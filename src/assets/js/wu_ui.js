...
(function () {
    ...
    Wu.prototype.showError = function (title) {
        wu.hideToast();
        var t = title || '缃戠粶璇锋眰瓒呮椂锛岃交瑙﹀睆骞曞埛鏂�';
        var el = document.createElement("div");
    ...
    Wu.prototype.showDialog = function (ops) {
        ...
        var h = '<div class="wu-mask-transparent wu-mask-black"></div>' +
            ...
            '<a class="wu-btn dialog-btn-cancel" style="' + (showCancel ? '' : 'display: none') + '">鍙栨秷</a>' +
            '<a class="wu-btn dialog-btn-confirm">纭</a>' +
            '</div></div>';
    ...
    Wu.prototype.showAction = function (ops) {
        ...
        h += '</div><div class="wu-btn wu-actionsheet-cancel">鍙栨秷</div></div>';
    ...
    Wu.prototype.handler = function (e, ops) {
        ...
        if (cls.indexOf('wu-btn wu-actionsheet-cell') >= 0) {
            ...
            if (val == "delete") {
                wu.showDialog({
                    title: '鎻愮ず',
                    content: ops.deleteText ? ops.deleteText : '纭畾鍒犻櫎鍚楋紵',
                    showCancel: true,
